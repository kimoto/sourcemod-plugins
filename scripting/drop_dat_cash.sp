...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_dosh", Command_Doshing);
...
public Action:Command_Doshing(client, args)
{
  if(!g_bInSetup){
    PrintToChat(client, "Setup time only!");
    return;
  }
  new drop_size = GetConVarInt(g_hDropAmount);
  if(args >= 1){
    decl String:buf[256];
    GetCmdArg(1, buf, sizeof(buf));
    LogMessage("arg: %s", buf);
    drop_size = StringToInt(buf);
  }

  //Drop his money - now!
	SpawnDosh(drop_size >= 200 ? CASH_LARGE : drop_size >= 100 ? CASH_MEDIUM : CASH_SMALL, client, drop_size );
  
  //Block command for 0.25sec 
	bBlocked[ client ] = true;
  
  //Unblock command after 0.25 sec
	CreateTimer( 0.25, Timer_UnblockCommand, client );
}
...
stock SpawnDosh( const String:strModel[], iClient, amount, const iColor[ 3 ] = {255, ...})
{
	//Get amount of cash
	new iAmount = GetEntProp( iClient, Prop_Send, "m_nCurrency" ) >= amount ? amount : GetEntProp( iClient, Prop_Send, "m_nCurrency" );
